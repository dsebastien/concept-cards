...
import { extractArticles } from '@/lib/extract-resources'
...
const ArticlesPage: React.FC = () => {
    ...
    // Extract all articles from concepts
    const allArticles = useMemo(() => {
        return extractArticles(conceptsData.concepts)
    }, [])
    ...
    const filteredArticles = useMemo(() => {
        ...
        if (searchQuery) {
            filtered = simpleFuzzySearch(filtered, searchQuery, (article) => article.title)
        }
        ...
        return filtered
    }, [allArticles, searchQuery, selectedType])
    ...
    useEffect(() => {
        if (selectedResource) {
            document.title = `${selectedResource.title} - Articles - Concepts`
        } else {
            document.title = 'Articles - Concepts'
        }
    }, [selectedResource])
    ...
    const handleShowDetails = useCallback((resource: ExtractedResource) => {
        ...
        window.history.pushState({}, '', `/articles/${resource.id}`)
    }, [])
    ...
    const handleNavigateToResource = useCallback((resource: ExtractedResource) => {
        setSelectedResource(resource)
        window.history.replaceState({}, '', `/articles/${resource.id}`)
    }, [])
    ...
    return (
        <AnimatedPage>
                ...
                <AnimatedHero className='mx-auto max-w-4xl'>
                    ...
                    <div className='flex items-center gap-4'>
                        <div className='flex h-14 w-14 items-center justify-center rounded-full bg-blue-500/10'>
                            <FaNewspaper className='h-7 w-7 text-blue-400' />
                        </div>
                        <div>
                            ...
                            <p className='text-primary/70 mt-1'>
                                Articles referenced across all concepts
                            </p>
                        </div>
                    </div>
                </AnimatedHero>
            ...
            >
                <div className='bg-primary/5 flex items-center justify-center gap-8 rounded-xl p-4'>
                    <div className='text-center'>
                        ...
                        />
                        <div className='text-primary/60 text-sm'>Total Articles</div>
                    </div>
                            ...
                            {filteredArticles.map((article, index) => (
                                ...
                                >
                                    <ResourceCard
                                        ...
                                        icon={<FaNewspaper className='h-5 w-5 text-blue-400' />}
                                    />
                                </motion.div>
                            ))}
                        </div>
            ...
            <ResourceDetailModal
                ...
                allResources={filteredArticles}
                isOpen={!!selectedResource}
                onClose={handleCloseDetails}
                onNavigateToResource={handleNavigateToResource}
                resourceType='article'
                icon={<FaNewspaper className='h-7 w-7 text-blue-400' />}
